...
class QRCodeGenerator extends CWidget {
    ...
    public function init()
    {
        ...
        if (!$this->filePath){
            $this->filePath = realpath(Yii::app()->getBasePath().'/../static/dindik/qrcode');
        }
        ...
        if (!isset($this->fileUrl)){
            $this->fileUrl = Yii::app()->baseUrl . '/static/dindik/qrcode';
        }
    ...
    public function run()
    {
		...
		//if($_SERVER['HTTP_HOST']==Yii::app()->params['qrdindik']){}
		if($_SERVER['HTTP_HOST']=='localhost'){
			$qr="http://localhost/dindik/cek/token/";
		}
		else{
			$qr="";
		}		
        QRcode::png($qr.''.$this->data, $this->filePath, $this->errorCorrectionLevel, $this->matrixPointSize, false);
